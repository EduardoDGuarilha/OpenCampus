...
const COURSE_CRITERIA: ReadonlyArray<{ key: CriterionKey; label: string }> = [
...
export default function CoursePage() {
  ...
  useEffect(() => {
    if (numericCourseId === null) {
      setPageError("Curso não encontrado.");
      setCourse(null);
      setInstitution(null);
      setMetrics(null);
      setProfessors([]);
    ...
    async function loadBaseData() {
      ...
      setPageError(null);
      try {
        const courseResponse = await httpClient.get<CourseResponse>(`/courses/${numericCourseId}`);
        if (cancelled) {
          return;
        }
        const courseData = courseResponse.data;

        const [metricsResponse, professorsResponse, subjectsResponse, institutionResponse] = await Promise.all([
          httpClient.get<MetricsResponse>("/reviews/metrics", {
            params: { target_type: "COURSE", target_id: courseData.id },
          }),
          httpClient.get<ProfessorResponse[]>("/professors", {
            params: { course_id: courseData.id },
          }),
          httpClient.get<SubjectResponse[]>("/subjects", {
            params: { course_id: courseData.id },
          }),
          httpClient.get<InstitutionResponse>(`/institutions/${courseData.institution_id}`),
        ]);
        ...
        const sortedProfessors = professorsResponse.data
          .slice()
          .sort((a, b) => a.name.localeCompare(b.name, "pt-BR"));
        const sortedSubjects = subjectsResponse.data
          .slice()
          .sort((a, b) => a.name.localeCompare(b.name, "pt-BR"));
    ...
    async function loadReviews() {
      ...
      try {
        const response = await httpClient.get<ReviewResponse[]>("/reviews", {
          params: { target_type: "COURSE", target_id: numericCourseId },
        });
        if (cancelled) {
          return;
        }
    ...
    async function loadComments() {
      ...
      try {
        ...
        const map = entries.reduce<Record<number, CommentResponse[]>>((accumulator, [reviewId, comments]) => {
          const sortedComments = comments.slice().sort((a, b) => a.created_at.localeCompare(b.created_at));
          accumulator[reviewId] = sortedComments;
          return accumulator;
        }, {});
  ...
  const courseName = course?.name ?? (numericCourseId ? `Curso #${numericCourseId}` : "Curso");
  ...
  return (
    <div className="space-y-8">
      <header className="space-y-2">
        <h1 className="text-3xl font-semibold text-slate-900">{courseName}</h1>
        <p className="text-sm text-slate-600">
          Métricas consolidadas consideram apenas avaliações aprovadas, preservando o anonimato de estudantes.
        </p>
        {institution && (
          <p className="text-xs text-slate-500">
            <span className="font-semibold text-slate-700">Vinculado à instituição:</span>{" "}
      ...
      <section className="grid gap-6 md:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900">Indicadores do curso</h2>
              <p className="mt-1 text-xs text-slate-500">Apenas avaliações aprovadas alimentam os dados abaixo.</p>
            </div>
            {isLoadingPage ? (
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-lg font-semibold text-slate-900">Professores associados</h2>
            <p className="mt-1 text-xs text-slate-500">
              Navegue para acompanhar indicadores específicos de docentes moderados pela comunidade.
            </p>
            {isLoadingPage ? (
              ...
              <p className="mt-4 text-sm text-slate-600">Nenhum professor cadastrado para este curso.</p>
            )}
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-lg font-semibold text-slate-900">Disciplinas vinculadas</h2>
            <p className="mt-1 text-xs text-slate-500">
              Descubra disciplinas avaliadas neste curso e acompanhe suas métricas aprovadas.
            </p>
            {isLoadingPage ? (
              <div className="mt-4 space-y-2">
                <div className="h-4 w-3/4 rounded bg-slate-100" />
              ...
              <p className="mt-4 text-sm text-slate-600">Nenhuma disciplina cadastrada para este curso.</p>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Conteúdo moderado preserva o anonimato do autor. Comentários oficiais aparecem com destaque.
            </p>
          </div>
        </div>
        ...
        {isLoadingReviews ? (
                ...
                <article key={review.id} className="space-y-4 rounded-2xl border border-slate-200 p-5">
                  ...
                  <div className="space-y-3 rounded-xl border border-slate-100 bg-slate-50 p-4">
                    <div className="flex items-center justify-between">
                      ...
                      {isLoadingComments && (
                        <span className="text-xs text-slate-500">Carregando comentários…</span>
                      )}
                    </div>
          ...
          <p className="mt-6 text-sm text-slate-600">
            Ainda não existem avaliações aprovadas para este curso. Compartilhe sua experiência para apoiar outros estudantes.
          </p>
        )}
      </section>
    </div>
