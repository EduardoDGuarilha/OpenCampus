...
const PROFESSOR_CRITERIA: ReadonlyArray<{ key: CriterionKey; label: string }> = [
...
export default function ProfessorPage() {
  ...
  const numericProfessorId = useMemo(() => {
    if (!professorId) {
      return null;
    }
  ...
  useEffect(() => {
    if (numericProfessorId === null) {
      setPageError("Professor não encontrado.");
      setProfessor(null);
      setCourse(null);
      setInstitution(null);
      setMetrics(null);
    ...
    async function loadBaseData() {
      ...
      try {
        const professorResponse = await httpClient.get<ProfessorResponse>(`/professors/${numericProfessorId}`);
        if (cancelled) {
          return;
        }
        ...
        const [metricsResponse, courseResponse] = await Promise.all([
          httpClient.get<MetricsResponse>("/reviews/metrics", {
            params: { target_type: "PROFESSOR", target_id: professorData.id },
          }),
          httpClient.get<CourseResponse>(`/courses/${professorData.course_id}`),
        ]);
    ...
    async function loadReviews() {
      ...
      try {
        const response = await httpClient.get<ReviewResponse[]>("/reviews", {
          params: { target_type: "PROFESSOR", target_id: numericProfessorId },
        });
        if (cancelled) {
          return;
        }
  ...
  const professorName = professor?.name ?? (numericProfessorId ? `Professor #${numericProfessorId}` : "Professor");
  ...
  return (
    <div className="space-y-8">
      <header className="space-y-2">
        <h1 className="text-3xl font-semibold text-slate-900">{professorName}</h1>
        <p className="text-sm text-slate-600">
          Somente avaliações aprovadas compõem as métricas exibidas abaixo, sempre preservando o anonimato estudantil.
        </p>
        {course && (
      ...
      <section className="grid gap-6 md:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-900">Indicadores do professor</h2>
          ...
          {isLoadingPage ? (
            ...
            <div className="mt-4 space-y-4">
              ...
              <dl className="grid gap-3 sm:grid-cols-2">
                {PROFESSOR_CRITERIA.map((criterion) => {
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-slate-900">O que você encontra aqui</h2>
          <p className="mt-2 text-sm text-slate-600">
            Esta página reúne impressões anônimas de estudantes já moderadas e comentários oficiais da instituição quando
            disponíveis.
          </p>
          <p className="mt-2 text-xs text-slate-500">
            Comentários e avaliações pendentes só aparecem para quem opta por visualizá-los explicitamente em outras seções.
          </p>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              A identidade do autor permanece protegida. Comentários de fontes oficiais aparecem com sinalização específica.
            </p>
          </div>
        </div>
        ...
        {isLoadingReviews ? (
          ...
          <p className="mt-6 text-sm text-slate-600">
            Ainda não existem avaliações aprovadas para este professor. Compartilhe sua experiência quando a moderação estiver
            disponível.
          </p>
        )}
      </section>
